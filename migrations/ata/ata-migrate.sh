...
WHAT WILL BE BACKED UP:
  ...
  • Everything to /arch-migration-backup-YYYYMMDD-HHMMSS"
    ...
    # Show summary
    local de flatpak_count snap_count
    ...
    # Convert before removing systemd (we need systemd tools for parsing)
    ata_convert_all

    # Handle systemd-homed users before removing systemd
    ...
    # Reinstall packages from Artix repos
    ...
    # Convert systemd --user services to autostart
    ata_convert_user_services

    # Handle systemd-boot → GRUB
    ata_convert_systemd_boot

    # DKMS rebuild
    ata_rebuild_dkms

    # Restore user data and configs
    ata_restore_user_data "${backup_dir}"

    # Restore flatpaks
    ata_restore_flatpaks "${backup_dir}"

    # Restore network credentials to target
...
AFTER REBOOT:
  • Check services: $(case "${target_init}" in openrc) echo "rc-update show -v";; runit) echo "ls /etc/runit/runsvdir/default/";; dinit) echo "dinitctl list";; s6) echo "s6-rc-db list services";; esac)
