...
if ! command -v "${FILLY_BIN}" &>/dev/null; then
    if [[ -f /usr/local/bin/filly ]]; then
        FILLY_BIN="/usr/local/bin/filly"
    else
        die "FILLY not found. Install the filly package or place the binary at /usr/local/bin/filly"
    fi
fi
